...
from senteval.tools.relatedness import HyperRelatednessPytorch
from senteval.tools.validation import HyperSplitClassifier


class SICKRelatednessEval(object):
    def __init__(self, task_path, seed=1111):
        print('***** Transfer task : SICK-Relatedness*****\n\n')
        self.seed = seed
    ...
    def run(self, params, batcher, space_search):
        ...
        for key in self.sick_data:
            ...
            for txt_type in ['X_A', 'X_B']:
                ...
                for ii in tqdm.tqdm(np.arange(0, len(self.sick_data[key]['y']), bsize)):
        ...
        start = time.time()
        clf = HyperRelatednessPytorch(train={'X': trainF, 'y': trainY},
                                 valid={'X': devF, 'y': devY},
                                 test={'X': testF, 'y': testY},
                                 devscores=self.sick_data['dev']['y'],
                                 config=config_classifier)

        devpr, yhat, best_params = clf.run()
        ...
        print('Dev : Pearson {0}'.format(devpr))
        print('Test : Pearson {0} Spearman {1} MSE {2} \
                       for SICK Relatedness\n'.format(pr, sr, se))

        return {'devpearson': devpr, 'pearson': pr, 'spearman': sr, 'mse': se,
                'yhat': yhat, 'ndev': len(devA), 'ntest': len(testA), 'best_params': best_params, 
...
class SICKEntailmentEval(SICKRelatednessEval):
    def __init__(self, task_path, seed=1111):
        print('***** Transfer task : SICK-Entailment*****\n\n')
        self.seed = seed
        train = self.loadFile(os.path.join(task_path, 'SICK_train.txt'))
        dev = self.loadFile(os.path.join(task_path, 'SICK_trial.txt'))
        test = self.loadFile(os.path.join(task_path, 'SICK_test_annotated.txt'))
    ...
    def run(self, params, batcher, space_search):
        ...
        start = time.time()
        clf = HyperSplitClassifier(X={'train': trainF, 'valid': devF, 'test': testF},
        ...
        return {'devacc': devacc, 'acc': testacc,
                'ndev': len(devA), 'ntest': len(testA), 'best_params': best_params, 
                'time_preprocess': time_preprocess, 'time_train': time_train}
