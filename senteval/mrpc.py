    ...
    def run(self, params, batcher, space_search):
        ...
        for key in self.mrpc_data:
            ...
            for txt_type in ['A', 'B']:
                ...
                for ii in tqdm.tqdm(np.arange(0, len(text_data['y']), params.batch_size)):
        ...
        start = time.time()
        clf = HyperClassifier(train={'X': trainF, 'y': trainY},
                              test={'X': testF, 'y': testY}, config=config_classifier)

        devacc, testacc, yhat, best_params = clf.run()
        
        time_train = time.time()-start
        
        testf1 = round(100*f1_score(testY, yhat), 2)
        print('Dev acc : {0} Test acc {1}; Test F1 {2} for MRPC.\n'
                      .format(devacc, testacc, testf1))
        return {'devacc': devacc, 'acc': testacc, 'f1': testf1,
                'ndev': len(trainA), 'ntest': len(testA), 'best_params': best_params, 
