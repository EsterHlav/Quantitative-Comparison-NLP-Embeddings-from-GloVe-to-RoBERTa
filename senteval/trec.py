...
from senteval.tools.validation import KFoldClassifier, HyperClassifier


class TRECEval(object):
    def __init__(self, task_path, seed=1111):
        print('***** Transfer task : TREC *****\n\n')
        self.seed = seed
    ...
    def run(self, params, batcher, space_search):
        ...
        for ii in tqdm.tqdm(np.arange(0, len(train_labels), params.batch_size)):
            batch = train_samples[ii:ii + params.batch_size]
            embeddings = batcher(params, batch)
            train_embeddings.append(embeddings)
        train_embeddings = np.vstack(train_embeddings)
        print('Computed train embeddings')
        ...
        print('Computed test embeddings')
        ...
        start = time.time()
        clf = HyperClassifier({'X': train_embeddings,
                               'y': np.array(train_labels)},
                              {'X': test_embeddings,
                               'y': np.array(test_labels)}, config=config_classifier)

        devacc, testacc, _, best_params = clf.run()
        time_train = time.time()-start
        
        print('\nDev acc : {0} Test acc : {1} \
            for TREC\n'.format(devacc, testacc))
        return {'devacc': devacc, 'acc': testacc,
                'ndev': len(self.train['X']), 'ntest': len(self.test['X']), 'best_params': best_params, 
