...
import logging
import numpy as np
from senteval.tools.classifier import MLP, MLPBayesSearch
...
class InnerKFoldClassifier(object):
    ...
    def run(self):
        print('Training {0} with (inner) {1}-fold cross-validation'
        ...
        for train_idx, test_idx in skf.split(self.X, self.y):
            ...
            print('Best param found at split {0}: l2reg = {1} \
...
class KFoldClassifier(object):
    ...
    def run(self):
        # cross-validation
        print('Training {0} with {1}-fold cross-validation'
                     .format(self.modelname, self.k))
        regs = [10**t for t in range(-5, -1)] if self.usepytorch else \
        ...
        # evaluation
        print([('reg:' + str(regs[idx]), scores[idx])
                      for idx in range(len(scores))])
        optreg = regs[np.argmax(scores)]
        devaccuracy = np.max(scores)
        print('Cross-validation : best param found is reg = {0} \
...
class HyperClassifier(object):
    ...
    def run(self):
        print('Training {0} with standard validation..'
                     .format(self.modelname))

        clf = MLPBayesSearch(inputdim=self.featdim, outputdim=self.outputdim, search_params=self.space_search, iter_bayes=self.iter_bayes, seed=self.seed, cudaEfficient=self.cudaEfficient)

        devacc = clf.fit(self.train['X'], self.train['y'], validation_split=0.15)
        ...
        return devacc, testaccuracy, yhat, best_params
...
class SplitClassifier(object):
    ...
    def run(self):
        ...
        optreg = regs[np.argmax(scores)]
        devaccuracy = np.max(scores)
        print('Validation : best param found is reg = {0} with score \
...
class HyperSplitClassifier(object):
    """
    (train, valid, test) split classifier with bayesian optimization (Hyperopt)
    """
    def __init__(self, X, y, config):
        # handle if not splited:
        if 'split' in config:
            self.split = config['split']
            N = X.shape[0]
            idx = np.arange(N)
            np.random.shuffle(idx)
            ntrain = int((1-sum(self.split))*N)
            ndev = int((1-self.split[0])*N)
            idx_train = idx[:ntrain]
            idx_valid = idx[ntrain:ndev]
            idx_test = idx[ndev:]
            X = {
                'train': X[idx_train],
                'valid': X[idx_valid],
    ...
    def run(self):
        ...
        testaccuracy = round(100*testaccuracy, 2)
        best_params = clf.best_params
        return devaccuracy, testaccuracy, best_params
