...
from senteval.tools.validation import SplitClassifier, HyperSplitClassifier
import sys
...
class SSTEval(object):
    def __init__(self, task_path, nclasses=2, seed=1111):
        ...
        print('***** Transfer task : SST %s classification *****\n\n', self.task_name)
    ...
    def run(self, params, batcher, space_search):
        ...
        for key in self.sst_data:
            ...
            sst_embed[key]['X'] = []
            for ii in tqdm.tqdm(np.arange(0, len(self.sst_data[key]['y']), bsize)):
        ...
        start = time.time()
        clf = HyperSplitClassifier(X={'train': sst_embed['train']['X'],
        ...
        return {'devacc': devacc, 'acc': testacc,
                ...
                'ntest': len(sst_embed['test']['X']), 'best_params': best_params, 
