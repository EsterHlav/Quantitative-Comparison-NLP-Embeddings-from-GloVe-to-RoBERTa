...
class SE(object):
    ...
    def eval(self, name, LOGFILE=None):
        # evaluate on evaluation [name], either takes string or list of strings
        if (isinstance(name, list)):
            self.results = {x: self.eval(x, LOGFILE=LOGFILE) for x in name}
            return self.results
        ...
        if name == 'CR':
            ...
            from search_spaces import SEARCH_SICKE
            space_search = SEARCH_SICKE
        ...
        self.results = self.evaluation.run(self.params, self.batcher, space_search)
        
        if LOGFILE is not None:
            with open(LOGFILE, 'a') as f:
                f.write("==="*20+'\n')
                f.write("--> {} - Task {} done <--\n".format(str(datetime.now()), name))
                for k in self.results.keys():
                    if k in ['mse', 'acc', 'pearson', 'time_preprocess', 'time_train']:
                        f.write("{}: {}\n".format(k, self.results[k]))
                f.write('\n')
                f.close()
